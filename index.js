...
async function resolveModel(config) {
  ...
  switch (provider) {
    case "openai":
      if (!process.env.OPENAI_API_KEY) throw new Error("aucune clé API OpenAI définie");
      return openai.createOpenAI({ apiKey: process.env.OPENAI_API_KEY })(model);
    case "anthropic":
      if (!process.env.ANTHROPIC_API_KEY) throw new Error("aucune clé API Anthropic définie");
      return anthropic.createAnthropic({ apiKey: process.env.ANTHROPIC_API_KEY })(model);
    case "xai":
      if (!process.env.XAI_API_KEY) throw new Error("aucune clé API xAI définie");
    ...
    case "apple": {
      ...
      if (!available) throw new Error(reason || "Apple Intelligence est indisponible");
...
async function checkAppleAvailability() {
  ...
  try {
    ...
    return {
      available: false,
      reason: err instanceof Error ? err.message : "Apple Intelligence est indisponible"
    };
  }
}
  ...
  tools: [
    {
      ...
      execute: async ({ query }) => {
        ...
        if (!apiKey) {
          return {
            error: "La recherche web est indisponible — l'opérateur n'a pas configuré de TAVILY_API_KEY."
          };
        }
...
const DEFAULT_CONFIG = {
  version: 1,
  assistant: { name: "Alex", wakeWord: "alex", userGender: "unspecified", persona: "" },
  ...
  llm: { provider: "ollama", model: "llama3.2", ollamaBaseUrl: "http://localhost:11434/v1/" },
  tts: {
    engine: "system",
...
function secretsPresence() {
  return {
    ...
    GOOGLE_API_KEY: hasSecret("GOOGLE_API_KEY"),
    OPENCODE_ZEN_API_KEY: hasSecret("OPENCODE_ZEN_API_KEY"),
...
async function synthesizeSpeech(text) {
  ...
  if (!trimmed) throw new Error("Texte manquant pour la synthèse.");
...
async function transcribe(provider, wav) {
  switch (provider) {
    ...
    default:
      throw new Error(`Le fournisseur « ${provider} » n'est pas un fournisseur STT cloud.`);
  }
}
...
function initAutoUpdater() {
  ...
  autoUpdater.on("update-downloaded", (info) => {
    ...
    void electron.dialog.showMessageBox({
      ...
      message: `Alex ${info.version} est prête à être installée.`,
      detail: "Redémarre pour appliquer la mise à jour."
    }).then(({ response }) => {
...
function createWindow() {
  const win = new electron.BrowserWindow({
    ...
    title: "Alex",
    icon: node_path.join(__dirname, "../../resources/icon.png"),
    show: false,
...
function loadRenderer(win, hash) {
  const onLoadError = (err) => console.error("[opendex] failed to load renderer", { hash }, err);
...
function broadcastSessionState(state) {
  ...
  if (oriWindow && !oriWindow.isDestroyed()) {
    oriWindow.webContents.send(IPC.sessionChanged, state);
  }
}
...
function registerIpc() {
  ...
  electron.ipcMain.handle(
    ...
    async (event, opts) => {
      ...
      if (config.voice.mode !== "realtime") {
        throw new Error("La voix temps r00e9el not pas activ00e9e dans les param00e8tres.");
      }
  ...
  electron.ipcMain.on(IPC.orbAudioLevel, (_event, level) => {
    if (oriWindow && !oriWindow.isDestroyed()) {
...
function createTray() {
  ...
  const menu = electron.Menu.buildFromTemplate([
    { label: "Afficher Alex", click: () => summonWindow({ toggle: false }) },
    { type: "separator" },
